...
def mechanic_register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            latitude_decimal = Decimal(str(latitude)) if latitude else None
            longitude_decimal = Decimal(str(longitude)) if longitude else None
            ...
            mp = MechanicProfile.objects.create(
                ...
                approved=True  # Since your model default is True
            )
            
            messages.success(request, 'Mechanic registration successful!')
            return redirect('login')
        else:
